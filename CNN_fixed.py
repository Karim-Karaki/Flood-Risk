...
def amp_context(device):
    """Use CUDA autocast only when CUDA is available."""
...
def engineer_terrain_features(df):
    ...
    df['dtm_m']          = df['dtm'] / 10
    # Relative elevation signal: centered and standardized within each region.
    # This follows the hackathon guidance to use elevation smartly rather than raw absolute height.
...
# ── 4. Patch Dataset — spatial split to avoid leakage ─────────────────
class FloodPatchDataset(Dataset):
    def __init__(self, feat_grid, label_grid, patch_size=11,
                 ...
                 split_mode='spatial', split_axis='x'):
        self.feat  = feat_grid
        ...
        if val_fraction > 0:
            if split_mode == 'random':
                ...
                # Spatial holdout: keep a contiguous strip out of training.
                # This is much closer to the hackathon recommendation than random pixel splitting.
                if split_axis == 'y':
                    coord = np.array([i for i, _ in all_positions])
                else:
                    coord = np.array([j for _, j in all_positions])

                unique_coords = np.sort(np.unique(coord))
                if len(unique_coords) < 2:
                    selected = all_positions
                else:
                    cut = int(np.floor(len(unique_coords) * (1 - val_fraction)))
                    cut = int(np.clip(cut, 1, len(unique_coords) - 1))
                    val_coords = set(unique_coords[cut:])
                    if is_val:
                        selected = [pos for pos in all_positions if (pos[0] if split_axis == 'y' else pos[1]) in val_coords]
                    else:
                        selected = [pos for pos in all_positions if (pos[0] if split_axis == 'y' else pos[1]) not in val_coords]
        else:
            selected = all_positions
...
scaler = make_grad_scaler(device)  # mixed precision for CUDA; plain training on CPU
